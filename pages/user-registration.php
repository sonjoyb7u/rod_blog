...
if(isset($UserRegister)) {
    ...
    $UploadImageDir = "admin/uploads/admin-image/" . $NewUserPhotoName;
    ...
    if(empty($FullName) || empty($Username) || empty($UserEmail) || empty($UserQualification) || empty($UserPassword) || empty($UserConfirmPassword) || empty($UserMobile) || empty($UserPhotoName) || empty($UserAddress) || empty($UserRole)) {
        ...
        if(checkUserNameExists($Username)) {
            ...
            $sql1 = "INSERT INTO `tbl_users`(`full_name`, `user_name`, `user_email`, `user_qualification`, `user_password`, `user_confirm_password`, `user_mobile`, `user_image`, `user_address`, `role`, `token`, `datetime`) VALUES (:fullName, :userName, :userEmail, :userQualification, :userPassword, :userConfirmPassword, :userMobile, :userImage, :userAddress, :userRole, :tokeN, :dateTime)";
...
////
            ...
            $stmt1->bindValue(':userRole', $UserRole);
            $stmt1->bindValue(':tokeN', $StrToken);
            ...
            if($InsertData) {
                $_SESSION['SuccessMsg'] = "Success, You registration has been successfully done, Please verify your email And Contact to Administrator for Login Approval!";
                redirect_to("index.php");

            } else {
                $_SESSION['ErrorMsg'] = "Something wrong, Registration failed and contact to the Administration!";
                ...
                <div class="box">
                    ...
                    <div class="panel mb-none">
                        <div class="panel-content bg-scale-0">
                            <form action="<?= htmlspecialchars($_SERVER['PHP_SELF'] . '?page=user-registration')?>" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    <span class="input-with-icon">
<!--                                        <textarea name="UserAddress" id="UserAddress" class="form-control" cols="30" rows="5" placeholder="Enter Your Address">-->
<!--                                        </textarea>-->
...
<!--                                    <a href="" class="btn btn-primary btn-block RegisterBtn">Register</a>-->
                                ...
                                <div class="form-group text-center LogForPassRegBtn">
                                    Have an account ? Please <a href="<?= htmlspecialchars($_SERVER['PHP_SELF'] . '?page=user-login')?>">Log In</a>
                                </div>
                            </form>
                        </div>
                    </div>
